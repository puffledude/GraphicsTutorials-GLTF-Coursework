...
OGLRenderer::OGLRenderer(Window &window)	{
	...
	currentShader = 0;							//0 is the 'null' object name for skeletonShader programs...

	window.SetRenderer(this);					//Tell our window about the new renderer! (Which will in turn resize the renderer window to fit...)
}

/*
Destructor. Deletes the default skeletonShader, and the OpenGL rendering context.
*/
...
/*
Updates the uniform matrices of the current skeletonShader. Assumes that
the skeletonShader has uniform matrices called modelMatrix, viewMatrix,
...
void OGLRenderer::SetShaderLight(const Light& l) {
	...
	glUniform4fv(glGetUniformLocation(currentShader->GetProgram(), "lightSpec"), 1, (float*)&l.GetSpec());
