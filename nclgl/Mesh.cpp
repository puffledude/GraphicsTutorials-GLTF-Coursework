...
void Mesh::SetBindPose(const std::vector<Matrix4>& newMats) {
	delete bindPose;
	bindPose = new Matrix4[newMats.size()];

	memcpy(bindPose, newMats.data(), sizeof(Matrix4) * newMats.size());
}

void Mesh::SetInverseBindPose(const std::vector<Matrix4>& newMats) {

	delete inverseBindPose;
	inverseBindPose = new Matrix4[newMats.size()];

	memcpy(inverseBindPose, newMats.data(), sizeof(Matrix4) * newMats.size());

}


bool Mesh::GetVertexIndiciesForTri(unsigned int i, unsigned int& a, unsigned int& b, unsigned int& c) const {
...
void Mesh::GenerateNormals() {
	...
	for (int i = 0; i < triCount; ++i) {
		...
		GetVertexIndiciesForTri(i, a, b, c);
