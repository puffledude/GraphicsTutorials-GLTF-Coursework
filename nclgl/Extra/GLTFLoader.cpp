...
bool GLTFLoader::Load(const std::string& filename, GLTFScene& intoScene) {
	...
	if (!gltf.LoadASCIIFromFile(&model, nullptr, nullptr, filename)) {
		return false;
	}

	BaseState state;
	state.firstAnim		= (uint32_t)intoScene.animations.size();
	state.firstMat		= (uint32_t)intoScene.materials.size();
	state.firstMatLayer = (uint32_t)intoScene.materialLayers.size();
	state.firstMesh		= (uint32_t)intoScene.meshes.size();
	state.firstNode		= (uint32_t)intoScene.sceneNodes.size();
	state.firstTex		= (uint32_t)intoScene.textures.size();

	LoadImages(model, intoScene, state, filename);
	LoadMaterials(model, intoScene, state);
	LoadSceneNodeData(model, intoScene, state);

	LoadVertexData(model, intoScene, state);
	AssignNodeMeshes(model, intoScene, state);

	return true;
}

void GLTFLoader::LoadImages(tinygltf::Model& m, GLTFScene& scene, BaseState state, const std::string& rootFile) {
	std::map<std::string, SharedTexture> loadedTexturesMap;
	...
	for (const auto& i : m.images) {
		...
		//SharedTexture tex = SharedTexture(new OGLTexture(pathString));

		SharedTexture tex = OGLTexture::TextureFromFile(pathString);
...
void GLTFLoader::LoadMaterials(tinygltf::Model& m, GLTFScene& scene, BaseState state) {
	...
	for (const auto& m : m.materials) {
		...
		layer.metallicFactor	= (float)m.pbrMetallicRoughness.metallicFactor;
		layer.roughnessFactor	= (float)m.pbrMetallicRoughness.roughnessFactor;
		layer.alphaCutoff		= (float)m.alphaCutoff;
		...
		if (m.pbrMetallicRoughness.baseColorFactor.size() == 4) {
			layer.albedoColour.x = (float)m.pbrMetallicRoughness.baseColorFactor[0];
			layer.albedoColour.y = (float)m.pbrMetallicRoughness.baseColorFactor[1];
			layer.albedoColour.z = (float)m.pbrMetallicRoughness.baseColorFactor[2];
			layer.albedoColour.w = (float)m.pbrMetallicRoughness.baseColorFactor[3];
		}
...
void GLTFLoader::LoadVertexData(tinygltf::Model& model, GLTFScene& scene, BaseState state ) {
	for (const auto& m : model.meshes) {
		...
		for (const auto& p : m.primitives) {
			...
			if (p.material >= 0) { //can ever be false?
				matLayer = scene.materialLayers[state.firstMatLayer + p.material];
			}
			material.allLayers.push_back(matLayer);

			submeshes.emplace_back((int)firstIndex, (int)(vIndices.size() - firstIndex), (int)baseVertex);
		}

		OGLMesh* outMesh = ::Mesh::MeshFromVectors(
			vPositions,
			{},
			vTexCoords,
			vNormals,
		...
		SharedMesh m = std::make_shared<OGLMesh>(*outMesh);
	...
	for (int i = 0; i < m.nodes.size(); ++i) {
		...
		if (!fileNode.matrix.empty()) { //node can be defined with a matrix - never targeted by animations!
			...
			if (!fileNode.rotation.empty()) {
				Quaternion q((float)fileNode.rotation[0], (float)fileNode.rotation[1], (float)fileNode.rotation[2], (float)fileNode.rotation[3]);
				q.Normalise();
				rotation = Matrix4(q);
			}
...
void GLTFLoader::LoadSkinningData(tinygltf::Model& model, GLTFScene& scene, int32_t meshNode, int32_t skinID, BaseState state) {
	...
	OGLMesh& mesh = *sceneNode.mesh;

	GLTFSkin skinData;

	skinData.globalTransformInverse = sceneNode.worldMatrix.Inverse();
...
void GLTFLoader::LoadAnimationData(tinygltf::Model& model, GLTFScene& scene, BaseState state, OGLMesh& mesh, GLTFSkin& skinData) {
	...
	for (const auto& anim : model.animations) {
		...
		localMatrices.reserve(jointCount * int(animLength * frameRate));

		std::vector<Matrix4> worldMatrices;
		worldMatrices.reserve(jointCount * int(animLength * frameRate));
		...
		while (time <= animLength) {
			...
			size_t startMatrix = localMatrices.size();
